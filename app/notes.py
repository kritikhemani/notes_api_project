...
async def create_note(request: Request, payload: NoteCreate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
...
@limiter.limit("5/minute")
async def read_notes(request: Request, skip: int = Query(0, ge=0), limit: int = Query(10, le=100), db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
